...
public class MapChunkData
{
	...
	public static MapChunkData create(World world) {
		MapChunkData newData = new MapChunkData(world);
	...
	private MapChunkData(World world) {
		this.world = world;
	}
...
//		Bukkit.getByUuid().info("x: "+x+"  z: "+z+"  offset: "+coordToRegionOffset(x, z));
	...
	private List<Boolean> getRegionData(CoordXZ region) {
		...
		// data for the specified region isn't a yet, so init it as empty and try to find the file and load the data
...
//		testImage(region, data);
	...
	private void sendMessage(String text) {
		Bukkit.getLogger().info("[UHC] [map] " + text);
	}
